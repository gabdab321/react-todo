...
const Form = ({addTodo}) => {
    const {showDone, setShowDone, showFailed, setShowFailed} = useContext(Context)
    ...
    return (
        <>
            ...
            <p style={{marginTop: "15px"}} className={cl.switch__label}>Show failed todos</p>
            <Switch className={cl.switch} checked={showFailed} onChange={() => setShowFailed(!showFailed)}/>
        </>
    );
};
